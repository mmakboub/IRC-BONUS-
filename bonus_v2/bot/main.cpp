...
/*   Updated: 2023/08/12 23:24:28 by mmakboub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
    ...
    while (1) {
        ...
        if(cmd =="play" || cmd =="Play")
        {
            ...
            // std::string usersChannel = "NAMES " + std::string(CHANNEL) + "\n";
            // send(sock,usersChannel.c_str(), usersChannel.size(), 0);
            while(flag == false)
            {
